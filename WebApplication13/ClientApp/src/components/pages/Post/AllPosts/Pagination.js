...
class Pagination extends Component {
    ...
    render() {
        ...
        if (!this.totalRecords || this.totalPages === 1) return (<div> no </div>);
    ...
    async populateWeatherData() {                            //методзапроса на сервер 
        const page = encodeURIComponent(this.props.match.params.page); // TODO check is integer
        // https://localhost:44394/post/all?page=1 
        const response = await fetch(`post/all?page=${page}`);         //асинхронный запрос - по маршруту: post - тип запроса GET
        const data = await response.json();


        // let totalPages =(int)Math.Ceiling((decimal)TotalItems / ItemsPerPage);
        let totalItems = 7;
        let totalPages = 3;
